...
(function() {
  window.onload = function() {
    var envMap, format, light, loader, manager, path;
    ...
    manager.onProgress = function(item, loaded, total) {
      return console.log(loaded / total * 100);
    };
    ...
    path = 'env/';
    format = '.jpg';
    envMap = new THREE.CubeTextureLoader().load([path + 'posx' + format, path + 'negx' + format, path + 'posy' + format, path + 'negy' + format, path + 'posz' + format, path + 'negz' + format]);
  ...
  window.onScroll = function() {
    if (document.body.offsetTop > 10) {
      return document.getElementById('down-arrow').classList.add('hidden');
    } else {
      return document.getElementById('down-arrow').classList.remove('hidden');
    }
  };

  window.mouseMove = function(event) {
    var cursor_position_x, cursor_position_y, normal_cursor_position_x, normal_cursor_position_y;
    ...
    return window.mouse.x = normal_cursor_position_x / 90;
  };
